...
document.addEventListener('DOMContentLoaded', function () {
  ...
  /* ---------- 2) Simulate click on correct avatar after form submission ---------- */
  (function avatarRoutingOnSubmit() {
    // mapping from (path, bodyType) -> lp-avatar value
    const AVATAR_MAP = {
      'male:overweight': 'Avatar 1', // Overweight Male
      'female:overweight': 'Avatar 2', // Overweight Female
      'male:soft': 'Avatar 3', // Soft Male
      'female:soft': 'Avatar 4', // Soft Female
      'male:skinny': 'Avatar 5' // Skinny Male
      // NOTE: no explicit mapping for female:skinny provided — handled below as fallback
    };

    let alreadyClicked = false;

    function getQuizState() {
      // Prefer localStorage state (your main script saves quizProgress there)
      try {
        const raw = localStorage.getItem('quizProgress');
        if (raw) {
          const j = JSON.parse(raw);
          if (j && (j.selectedPath || j.currentBodyType)) {
            return { selectedPath: j.selectedPath || null, currentBodyType: j.currentBodyType || null };
          }
        }
      } catch (e) {
      ...
      // DOM fallback: try detect selected gender and body-type from visible inputs
      try {
        // guess gender from visible wrapper or checked radio with attribute gender-path
        const genderRadio = document.querySelector('input[gender-path]:checked');
        const selectedPath = genderRadio ? genderRadio.getAttribute('gender-path') : (document.querySelector('.form_step_wrap.male')?.style.display === 'block' ? 'male' : (document.querySelector('.form_step_wrap.female')?.style.display === 'block' ? 'female' : null));

        // guess body type by checked input with attribute body-type
        const bodyRadio = document.querySelector('input[body-type]:checked');
        const currentBodyType = bodyRadio ? bodyRadio.getAttribute('body-type') : null;

        return { selectedPath, currentBodyType };
      } catch (e) {
        return { selectedPath: null, currentBodyType: null };
      }
    }

    function chooseAvatarName(state) {
      const path = (state.selectedPath || '').toString().toLowerCase();
      const body = (state.currentBodyType || '').toString().toLowerCase();

      // simple key
      const key = `${path}:${body}`;
      if (AVATAR_MAP[key]) return AVATAR_MAP[key];

      // fallback rules: if female+skinny not defined, pick overweight-female as fallback (safe)
      if (path === 'female' && body === 'skinny') {
        // choose Avatar 2 (overweight female) as fallback — change if you prefer another
        return AVATAR_MAP['female:overweight'] || null;
      }

      // fallback attempt: if any avatar exists for this gender (soft or overweight)
      const genderPreferredKeys = Object.keys(AVATAR_MAP).filter(k => k.startsWith(path + ':'));
      if (genderPreferredKeys.length) {
        // pick first available for that gender
        return AVATAR_MAP[genderPreferredKeys[0]] || null;
      }

      return null;
    }

    function simulateAvatarClickOnce() {
      if (alreadyClicked) return;
      alreadyClicked = true;

      const state = getQuizState();
      const avatarName = chooseAvatarName(state);

      if (!avatarName) {
        console.warn('No avatar mapping found for quiz state', state);
        return;
      }

      // find button with appropriate lp-avatar attribute
      const selector = `[lp-avatar="${avatarName}"], [lp-avatar='${avatarName}']`;
      const btn = document.querySelector(selector);

      if (btn) {
        // small delay to allow form libraries to finish processing (if needed)
        setTimeout(() => {
          try {
            btn.click();
            console.info('Simulated avatar click for', avatarName);
          } catch (e) {
            console.warn('Failed to click avatar button', e);
          }
        }, 120); // tiny delay
      } else {
        console.warn('Avatar button not found for', avatarName, 'selector:', selector);
      }
    }

    // Helper: try to detect form success. We'll use multiple signals to be robust:
    function bindSubmitAndSuccessDetectors() {
      // 1) On native form submit event (fires for normal submits)
      document.addEventListener('submit', function onSubmit(evt) {
        // run once per submit
        // wait a short time to allow success-state to be applied by form scripts
        setTimeout(simulateAvatarClickOnce, 250);
      }, true /* capture so we run early */);

      // 2) Attach click handler to any .submit_btn as requested, to trigger fallback detection
      document.querySelectorAll('.submit_btn').forEach(btn => {
        btn.addEventListener('click', function () {
          // if submission is via AJAX that doesn't fire 'submit', we also watch for success markers
          // Start a short-lived MutationObserver to watch for common success nodes
          const mo = new MutationObserver((mutations, observer) => {
            // Look for Webflow success node .w-form-done or any element added that contains 'thank' / 'success'
            const done = document.querySelector('.w-form-done, .form-success, .form__success, [data-form-success]');
            if (done) {
              observer.disconnect();
              setTimeout(simulateAvatarClickOnce, 120);
            } else {
              // small heuristic: check for text nodes with 'thank' or 'success' in new nodes
          ...
          // 3) Fallback timer: if none of the above fired, try once after 1.5s
          setTimeout(() => {
            simulateAvatarClickOnce();
            try { mo.disconnect(); } catch (e) {}
          }, 1500);
        }, { passive: true });
      });

      // 4) As an extra safety, listen for elements that become visible with a success class
      const globalObserver = new MutationObserver(() => {
        const done = document.querySelector('.w-form-done, .form-success, .form__success, [data-form-success]');
        if (done) simulateAvatarClickOnce();
      });
    ...
    try {
      bindSubmitAndSuccessDetectors();
    } catch (e) {
      console.warn('Failed to bind submit detectors', e);
    }
  })();
});
