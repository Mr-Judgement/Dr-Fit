...
(function () {
  // --- 1) fill language-type from URL ---
  function fillLanguageInputFromURL() {
    const input = document.getElementById('language-type');
    if (!input) return;

    // Normalize path and get first segment
    const firstSegment = (location.pathname || '').replace(/^\/+|\/+$/g, '').split('/')[0] || '';

    // Accept patterns like "en", "ro", "en-US" (basic validation)
    if (/^[a-z]{2}(-[A-Za-z]{2})?$/.test(firstSegment)) {
      input.value = firstSegment;
    } else {
      // If not found, leave blank (or optionally set a default)
      // input.value = 'en';
      input.value = '';
    }
  }

  // --- 2) choose avatar based on gender + body-type and click corresponding lp-avatar button ---
  const avatarMap = {
    male: {
      overweight: 'Avatar 1',
      soft: 'Avatar 3',
    ...
    female: {
      overweight: 'Avatar 2',
      soft: 'Avatar 4'
      // skinny female wasn't specified in your mapping — intentionally left out
    }
  };

  function determineAvatarLabel() {
    // 1) Try saved state in localStorage (your code writes quizProgress there)
    try {
      const saved = JSON.parse(localStorage.getItem('quizProgress') || '{}');
      const path = saved.selectedPath || null;           // "male" or "female"
      const body = saved.currentBodyType || null;       // "overweight" / "soft" / "skinny"
      if (path && body && avatarMap[path] && avatarMap[path][body]) {
        return avatarMap[path][body];
      }
    } catch (e) {
      // ignore malformed saved state
      console.warn('Could not parse quizProgress from localStorage', e);
    }

    // 2) Fall back to DOM - find checked gender and body-type inputs
    const genderInput = document.querySelector('input[gender-path]:checked');
    const bodyInput = document.querySelector('input[body-type]:checked');

    const path = genderInput?.getAttribute('gender-path') || null;
    const body = bodyInput?.getAttribute('body-type') || null;

    if (path && body && avatarMap[path] && avatarMap[path][body]) {
      return avatarMap[path][body];
    }

    // 3) If not found, attempt heuristic: look for any element that indicates chosen body-type in the active steps
    const activeBodyTypeNode = document.querySelector('.form_step.active [body-type].selected, .form_step.active input[body-type][checked]');
    if (activeBodyTypeNode) {
      const heuristicBody = activeBodyTypeNode.getAttribute && activeBodyTypeNode.getAttribute('body-type');
      const heuristicGender = document.querySelector('.form_step.active input[gender-path]')?.getAttribute('gender-path') || null;
      if (heuristicGender && heuristicBody && avatarMap[heuristicGender] && avatarMap[heuristicGender][heuristicBody]) {
        return avatarMap[heuristicGender][heuristicBody];
      }
    }
  ...
  function clickAvatarIfFound() {
    if (document.body.dataset._avatarClicked === 'true') return; // avoid double-clicks
    const label = determineAvatarLabel();
    if (!label) {
      console.warn('No avatar mapping could be determined for current path/body-type.');
      return;
    }

    const btn = document.querySelector(`[lp-avatar="${label}"]`);
    if (!btn) {
      console.warn('No button with lp-avatar="' + label + '" found in DOM.');
      return;
    }

    // mark so we don't click multiple times
    document.body.dataset._avatarClicked = 'true';
    try {
      // Use click() so Webflow/CMS bindings behave as if user clicked
      btn.click();
      console.log('Simulated click on avatar button:', label);
    } catch (e) {
      console.error('Error clicking avatar button:', e);
    }
  }

  // Watch for submit & success:
  function wireSubmitToAvatarClick() {
    // 1) Delegate click listener for .submit_btn (fires immediately on click)
    document.addEventListener('click', function (ev) {
      const s = ev.target.closest && ev.target.closest('.submit_btn');
      if (!s) return;
      // Give any synchronous submission handlers a tick, then attempt avatar routing
      setTimeout(clickAvatarIfFound, 50);
      // Also start observing for a success node in case submission is async and reveals a "done" element later
      observeForFormSuccessAndClick();
    }, true);

    // 2) If a form fires a 'submit' event, use that as another signal
    document.addEventListener('submit', function (ev) {
      // let the form submission proceed; still attempt to click avatar (use a tiny delay)
      setTimeout(clickAvatarIfFound, 100);
      observeForFormSuccessAndClick();
    }, true);

    // 3) If the page already shows a success node (e.g., user returns to page), try clicking immediately
    setTimeout(() => {
      if (document.querySelector('.w-form-done, .form-success, .submission-success, .thank-you, .success-message')) {
        clickAvatarIfFound();
      }
    }, 200);
  }

  // Look for common success nodes; when found run avatar click
  let successObserver = null;
  function observeForFormSuccessAndClick() {
    // If already observing do nothing
    if (successObserver) return;

    successObserver = new MutationObserver((mutations) => {
      for (const m of mutations) {
        // If any added node looks like a success message, trigger avatar click
        for (const n of m.addedNodes) {
          if (!(n instanceof HTMLElement)) continue;
          if (n.matches && n.matches('.w-form-done, .form-success, .submission-success, .thank-you, .success-message')) {
            clickAvatarIfFound();
            disconnectObserver();
            return;
          }
          // also check descendants
          if (n.querySelector && (n.querySelector('.w-form-done, .form-success, .submission-success, .thank-you, .success-message'))) {
            clickAvatarIfFound();
            disconnectObserver();
            return;
          }
        ...
        // If attributes changed on existing node and class added that looks like success
        if (m.type === 'attributes' && m.target instanceof HTMLElement) {
          const t = m.target;
          if (t.classList && /w-form-done|form-success|submission-success|thank-you|success-message/.test(t.className)) {
            clickAvatarIfFound();
            disconnectObserver();
            return;
          }
    ...
    successObserver.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ['class'] });

    // Safety: stop observing after 12s to avoid leak if nothing happens
    setTimeout(disconnectObserver, 12000);
  }

  function disconnectObserver() {
    if (!successObserver) return;
    try { successObserver.disconnect(); } catch (e) {}
